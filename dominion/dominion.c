...
int isGameOver(struct gameState *state) {
  ...
  j = 0;
  for (i = 0; i < 25; i++) // this is a bug, it should be 26
    {
...
int adventurerCard(int drawntreasure, struct gameState *state, int currentPlayer, int cardDrawn, int* temphand, int z){
    while(drawntreasure<2){
        ...
        if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
          drawntreasure += 2; // changed from 1 to 2
        else{
...
int great_hallCard(struct gameState* state, int handPos, int currentPlayer){
  //+1 Card
     drawCard(currentPlayer, state);
        
  //+1 Actions -- Added 2 more actions
     state->numActions++;
     state->numActions++;
     state->numActions++;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case great_hall:
        return great_hallCard(state, handPos, currentPlayer);
